...
void kernel_init_ht(__global char *ht)
{
    ...
    *(__global uint *)(ht + tid * (((1 << (((200 / (9 + 1)) + 1) - 20)) * 6) * 32)) = 0;
}
	...
	__global uint *debug)
{
    ...
    __global char       *p;
    uint                cnt;
    uchar		first_words[((1 << (((200 / (9 + 1)) + 1) - 20)) * 6)];
    uchar		mask;
    uint                i, j;
    // ((1 << (((200 / (9 + 1)) + 1) - 20)) * 6) is already oversized (by a factor of OVERHEAD), but we want to
    // make it even larger
    ushort		collisions[((1 << (((200 / (9 + 1)) + 1) - 20)) * 6) * 3];
    uint                nr_coll = 0;
    uint                n;
    uint		dropped_coll = 0;
    uint		dropped_stor = 0;
    ...
    cnt = *(__global uint *)p;
    cnt = min(cnt, (uint)((1 << (((200 / (9 + 1)) + 1) - 20)) * 6)); // handle possible overflow in prev. round
    if (!cnt)
    ...
    for (i = 0; i < cnt; i++)
      ...
      {
        a = (__global ulong *)
            (ht_src + tid * ((1 << (((200 / (9 + 1)) + 1) - 20)) * 6) * 32 + i * 32 + xi_offset);
        b = (__global ulong *)
            (ht_src + tid * ((1 << (((200 / (9 + 1)) + 1) - 20)) * 6) * 32 + j * 32 + xi_offset);
	dropped_stor += xor_and_store(round, ht_dst, tid, i, j, a, b);
      }
    if (round < 8)
	// reset the counter in preparation of the next round
	*(__global uint *)(ht_src + tid * ((1 << (((200 / (9 + 1)) + 1) - 20)) * 6) * 32) = 0;
}

__kernel __attribute__((reqd_work_group_size(64, 1, 1)))
void kernel_round1(__global char *ht_src, __global char *ht_dst, 
	__global uint *debug) 
{ 
    equihash_round(1, ht_src, ht_dst, debug); 
}
__kernel __attribute__((reqd_work_group_size(64, 1, 1)))
void kernel_round2(__global char *ht_src, __global char *ht_dst, 
	__global uint *debug) 
{ 
...
__kernel __attribute__((reqd_work_group_size(64, 1, 1)))
void kernel_round4(__global char *ht_src, __global char *ht_dst, 
	__global uint *debug) 
{ 
    equihash_round(4, ht_src, ht_dst, debug); 
}
__kernel __attribute__((reqd_work_group_size(64, 1, 1)))
void kernel_round5(__global char *ht_src, __global char *ht_dst, 
	__global uint *debug) 
{ 
...
void kernel_sols(__global char *ht0, __global char *ht1, __global sols_t *sols)
{
    ...
    uint		ht_i = (9 - 1) & 1; // table filled at last round
    uint		cnt;
    ...
    cnt = *(__global uint *)a;
    cnt = min(cnt, (uint)((1 << (((200 / (9 + 1)) + 1) - 20)) * 6)); // handle possible overflow in last round
    coll = 0;
    a += xi_offset;
