...
#define BOOST_LOG_CUSTOM BOOST_LOG_TRIVIAL(trace) << "pow | "
                               ...
                               const std::function<bool(EhSolverCancelCheck)> cancelled)
{
	...
	BOOST_LOG_CUSTOM << "Generating first list";
    ...
    if (X.size() > 1) {
		...
		BOOST_LOG_CUSTOM << "- List is empty";
...
invalidsolution:
	...
	BOOST_LOG_CUSTOM << "- Number of invalid solutions found: " << invalidCount;
...
bool Equihash<N,K>::IsValidSolution(const eh_HashState& base_state, std::vector<unsigned char> soln)
{
    if (soln.size() != SolutionWidth) {
		...
		BOOST_LOG_CUSTOM << "Invalid solution length: " << soln.size() << " (expected " << SolutionWidth << ")";
        return false;
    }
...
// Explicit instantiations for Equihash<48,3>
template int Equihash<48, 3>::InitialiseState(eh_HashState& base_state);
template bool Equihash<48, 3>::BasicSolve(const eh_HashState& base_state,
	const std::function<bool(std::vector<unsigned char>)> validBlock,
	const std::function<bool(EhSolverCancelCheck)> cancelled);
template bool Equihash<48, 3>::OptimisedSolve(const eh_HashState& base_state,
	const std::function<bool(std::vector<unsigned char>)> validBlock,
	const std::function<bool(EhSolverCancelCheck)> cancelled);
template bool Equihash<48, 3>::IsValidSolution(const eh_HashState& base_state, std::vector<unsigned char> soln);

// Explicit instantiations for Equihash<144,8>
template int Equihash<144, 8>::InitialiseState(eh_HashState& base_state);
template bool Equihash<144, 8>::BasicSolve(const eh_HashState& base_state,
	const std::function<bool(std::vector<unsigned char>)> validBlock,
	const std::function<bool(EhSolverCancelCheck)> cancelled);
template bool Equihash<144, 8>::OptimisedSolve(const eh_HashState& base_state,
	const std::function<bool(std::vector<unsigned char>)> validBlock,
	const std::function<bool(EhSolverCancelCheck)> cancelled);
template bool Equihash<144, 8>::IsValidSolution(const eh_HashState& base_state, std::vector<unsigned char> soln);