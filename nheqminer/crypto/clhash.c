...
// optimized only to carryless multiply a repeating sequence of the 64 bytes passed with the random source
static __m128i __clmulhalfscalarproductwithtailwithoutreduction64repeat(const __m128i * randomsource, const uint64_t buf[8])
{
    __m128i acc = _mm_setzero_si128();

    const __m128i *randomend = randomsource + 64;
    for (; randomsource < randomend; randomsource += 2)
    {
        const __m128i temp1 = _mm_load_si128(randomsource);
        const __m128i temp2 = _mm_lddqu_si128((__m128i *) buf);
        const __m128i add1 = _mm_xor_si128(temp1, temp2);
        const __m128i clprod1 = _mm_clmulepi64_si128(add1, add1, 0x10);
        acc = _mm_xor_si128(clprod1, acc);
        const __m128i temp12 = _mm_load_si128(randomsource+1);
        const __m128i temp22 = _mm_lddqu_si128((__m128i *) (buf + 2));
        const __m128i add12 = _mm_xor_si128(temp12, temp22);
        const __m128i clprod12 = _mm_clmulepi64_si128(add12, add12, 0x10);
        acc = _mm_xor_si128(clprod12, acc);
    }
